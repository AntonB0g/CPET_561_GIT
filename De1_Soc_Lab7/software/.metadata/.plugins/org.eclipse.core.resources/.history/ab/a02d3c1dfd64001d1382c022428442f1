...
void RAM_test(uint32 s_address, uint32 ram_size, uint32 test_data, uint32 access_size)
{
	switch(access_size){
	case 8:
			uint8 byte_data = uint8(test_data);
			uint32 byte_num = ram_size; // number 32 bit WORDS is 512 in 16 Bytes of data
			...
			break;
	case 16:
			uint16 half_data = uint16(test_data);
			uint32 halfs = ram_size/2; // number 32 bit WORDS is 512 in 16 Bytes of data
			...
			break;
	case 32:
			uint32 words = ram_size/4; // number 32 bit WORDS is 512 in 16 Bytes of data
